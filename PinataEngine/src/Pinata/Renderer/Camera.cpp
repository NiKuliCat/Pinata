...
namespace Pinata {
	Camera::Camera()
	{
		...
		CaculateProjectionMatrix(m_ProjectionMode, m_FOV, m_Aspect, m_Near, m_Far);
	}
	...
	void Camera::CaculateProjectionMatrix(ProjectionMode mode, float fov, float aspect, float nearPlane, float farPlane)
	{
		...
		{
			m_ProjectionMatrix = glm::ortho(-aspect * fov, aspect * fov, -fov, fov);
		}
	}
}